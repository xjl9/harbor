...
// Logic mirrors src/components/onboarding/stremio-step.tsx: intro/form/success
// with useAuth().signIn. Mobile tweaks: 16px inputs (iOS zoom floor), no
// autoFocus on entry (would throw the keyboard before the slide lands).
export function ObStremio() {
    ...
    return (
      <form key="form" onSubmit={submit} className={`flex flex-col gap-5 ${animClass}`}>
        <button
          type="button"
          onClick={goToIntro}
          className="flex h-11 w-fit items-center gap-1.5 text-[12.5px] font-medium text-ink-subtle transition-colors active:text-ink-muted"
        >
        ...
        <div className="flex flex-col gap-3">
          <FormField label={t("Email")} type="email" value={email} onChange={setEmail} disabled={busy} />
          <FormField
            label={t("Password")}
            type="password"
            value={password}
  ...
  return (
    <div key="intro" className={`flex flex-col gap-6 ${animClass}`}>
      ...
      <div className="flex flex-col gap-2">
        <button
          onClick={goToForm}
          className="flex h-14 items-center justify-center gap-3 rounded-2xl border border-edge bg-canvas/60 px-6 transition-all active:border-ink-subtle/40 active:bg-canvas"
        >
        ...
        <button
          onClick={() => openUrl("https://www.stremio.com/register")}
          className="min-h-11 text-center text-[12.5px] text-ink-subtle transition-colors active:text-ink-muted"
        >
...
// Duplicated from stremio-step.tsx FormField/StremioAvatar (not exported there);
// dedup into a shared module next time the desktop onboarding files are open.
function FormField({
  label,
  type,
  value,
  ...
  return (
    <label className="flex flex-col gap-1.5">
      ...
      <input
        ...
        className="h-12 rounded-xl border border-edge bg-canvas px-4 text-[16px] text-ink outline-none transition-colors focus:border-[#8b5cff]/60 disabled:opacity-50"
      />
    </label>
  );
}
