const misc: Record<string, string> = {
  ...
  "Addons are where catalogs and streams come from, and Harbor starts with none. Add one now so the first title you open has something to play.":
    "Дополнения предоставляют каталоги и потоки, а в Harbor поначалу нет ни одного. Добавьте дополнение сейчас, чтобы уже при открытии первого фильма или сериала было что воспроизвести.",
  "Could not install that addon.": "Не удалось установить это дополнение.",
  "Add or remove sources anytime from Profile, under Addons.":
    "Источники можно добавить или удалить в любой момент в Профиле, раздел «Дополнения».",
  "Not sure which? Torrentio and Cinemeta cover most people. You can change this later in Profile, under Addons.":
    "Не знаете, что выбрать? Torrentio и Cinemeta подойдут большинству пользователей. Изменить выбор можно позже в Профиле, раздел «Дополнения».",
  ...
  "Connect a debrid provider to see cached streams that can start immediately. This is optional, requires a separate paid provider account, and can be configured later in Settings.":
    "Подключите debrid-провайдера, чтобы видеть кэшированные потоки, которые запускаются сразу. Это необязательно, требует отдельной платной учётной записи провайдера и настраивается позже в Настройках.",
  ...
  "No debrid? Skip this. Harbor still finds free public streams, they can just be slower.":
    "Нет debrid? Пропустите этот шаг. Harbor всё равно найдёт бесплатные общедоступные потоки, но они могут быть медленнее.",
