...
function Photo({ person, className }: { person: XrayPerson; className: string }) {
  ...
  return (
    <div
      aria-hidden="true"
      className={`${className} flex items-center justify-center bg-white/[0.06] text-white/40`}
    >
...
export function XrayRailCard({
  ...
  return (
    <button
      ...
      className="flex w-full items-center gap-3 rounded-lg px-1.5 py-1.5 text-start hover:bg-white/10 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white"
    >
      ...
      <div className="flex min-w-0 flex-col">
        ...
        {person.sub && (
          <span className="truncate text-[12px] leading-tight text-white/55">{person.sub}</span>
        )}
      </div>
    </button>
  );
...
export function XrayTile({
  ...
  return (
    <button
      ...
      onKeyDown={(e) => e.stopPropagation()}
      onKeyUp={(e) => e.stopPropagation()}
      className="group flex w-full flex-col gap-2.5 rounded-lg text-start focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white"
    >
      ...
      <div className="flex flex-col gap-0.5">
        <span className="truncate text-[14px] font-semibold leading-tight text-white transition-colors group-hover:text-white">
          {person.name}
        </span>
        {person.sub && (
          <span className="line-clamp-1 text-[12.5px] leading-tight text-white/55">
            {person.sub}
          </span>
        )}
      </div>
